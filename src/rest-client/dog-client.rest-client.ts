...
export class DogClient {
  ...
  public async getTerriers(): Promise<{data: string[];}> {
    ...
    const request = this.httpService.get(this.baseUrl+'breeds/list/all').pipe(
      map(res => res.data?.message),
      map(message => message?.terrier)
    ).pipe(
      catchError(() => {
        const responseBody = ResponseUtils.callResponse(ConstMessages.BAD_REQUEST,{ msn: "Error called get terriers appi"},HttpStatus.FORBIDDEN);
